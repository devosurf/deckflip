...
| Speaker notes | Represented | `aside.notes` | notes slide re-emitted verbatim when untouched; else regenerated on the preserved notes master (its own if the deck has none). Notes are never measured, so the regenerated body carries text, emphasis, links, alignment and list levels, and the notes master governs typeface, size, colour and spacing |
