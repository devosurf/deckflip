...
- Wrap-width guard (the +/-0.5 px tolerance from #4), both directions: for each measured line, if `available width - line ink width < 0.5 px`, the box is widened by 1 px on the trailing side (0.5 px each side for centred text) with insets unchanged; conversely, if giving the block 0.5 px more moves any of Chromium's breaks (the next word missed by under 0.5 px), the trailing inset grows by 1 px (0.5 px each side when centred). The visual shift is at most 0.5 px; the guard removes the cases where sub-pixel glyph-advance differences would produce a different break in PowerPoint. Text-bearing `roundRect`/`ellipse` shapes are emitted as a `custGeom` with a full text rectangle: the presets inset their text rectangle by the corner, which would change the wrap width.
